...
<div id="pagination" class="pagination">
    <c:if test="${pageInfo.hasPrevious}">
        <a href="${pageContext.request.contextPath}${baseUrl}${baseUrl.contains('?') ? '&' : '?'}page=${pageInfo.currentPage - 1}"
           class="btn btn-outline">&lt; 이전</a>
    </c:if>
    <c:forEach var="i" begin="1" end="${pageInfo.totalPages}">
        <a href="${pageContext.request.contextPath}${baseUrl}${baseUrl.contains('?') ? '&' : '?'}page=${i}"
